        ...
        for message in messages:
            ...
            if 'replies' in message:
                for reply in message['replies']:
                    reply_timestamp = format_timestamp(reply['ts'])
...
def summarize_slack_channel(channel_url, start_date, end_date, session):
    if not channel_url:
        logging.error("Slack channel URL not provided")
        return "No channel URL provided."

    channel_text = get_channel_messages(channel_url, start_date, end_date)
    if channel_text:
        logging.info("Summarizing channel content")
        prompt = f"""
        Summarize the following Slack conversation, focusing on anything that seems particularly relevant or any patterns you notice:

        Slack conversation:
        {channel_text}
        """
        summary = Complete(model='llama3-70b', prompt=prompt, session=session)
        return summary
    else:
        logging.warning("No messages found or error occurred")
...
def main():
    ...
    if not SLACK_CHANNEL:
        logging.error("SLACK_CHANNEL environment variable is not set")
        return

    logging.info(f"Summarizing Slack channel: {SLACK_CHANNEL}")
    summary = summarize_slack_channel(SLACK_CHANNEL, START_DATE, END_DATE, session)
